...
export const apiKey = process.env.NEXT_PUBLIC_TMDB_API_TOKEN
...
export const authentication = async(): Promise<string> => {
    try {
        const response = await fetch(`${baseUrl}/authentication/guest_session/new`, {
            headers: {
                'Authorization': `Bearer ${apiKey}`,
                'Content-Type': 'application/json'
            },
